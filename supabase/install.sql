...
-- RPC code-gardée : le client la voit tout de suite (client_space relit le même
-- coffre) et elle remonte au conducteur (même stockage). 'client-espace' = client.
...
-- ----------------------------------------------------------------------------
-- Écriture client : RÉPONDRE avec un DOCUMENT (durable) — code vérifié serveur
-- ----------------------------------------------------------------------------
...
returns jsonb language plpgsql security definer set search_path = public, extensions as $$
declare v_hash text; v_ok boolean; v_doc_id uuid; v_libelle text;
begin
  select code_hash into v_hash from project_client_access where project_id = p_project;
  if v_hash is null or v_hash <> crypt(p_code, v_hash) then
    raise exception 'forbidden: bad code' using errcode = '42501';
  ...
  select exists (
    select 1 from event where id = p_event and project_id = p_project and type = 'demande'
      and visibility = 'client' and (content ->> 'destinataire') = 'client' and state = 'ouverte'
  ) into v_ok;
  if not v_ok then raise exception 'demande introuvable ou deja traitee' using errcode = '42501'; end if;
  ...
  insert into event(project_id, type, author_id, author_role, visibility, state, content)
  values (p_project, 'document', null, 'client', 'client', 'publie',
          jsonb_build_object('attachment', p_doc, 'libelle', v_libelle))
  returning id into v_doc_id;
  update event
     set content = content || jsonb_build_object('resolution', jsonb_build_object(
           'texte', coalesce(btrim(p_texte), ''), 'docEventId', v_doc_id,
           'resolvedAt', to_char(now() at time zone 'utc', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"'),
           'resolvedBy', (select client_id from project where id = p_project))),
         state = 'traitee'
   where id = p_event;
...
$$;
grant execute on function client_respond_document(uuid, text, uuid, text, jsonb, text) to anon, authenticated;
