import { Banknote, FileText, Lock, Palette, TriangleAlert } from 'lucide-react';
import {
  devisTotals,
  devisVigilances,
  lotTotalHT,
 ...
 * totaux ventilés par taux. Chaque lot est une PORTE D'ENTRÉE vers ce que PHÉNIX
 * a préparé (commandes, choix client, documents, vigilances). Lecture seule : le
 * devis signé est IMMUABLE — on ne le « corrige » jamais ; une erreur se traite
 * en vigilance ou en avenant.
 */
  ...
  return (
    <div className="space-y-3">
      {devis.lots.map((lot) => {
        ...
        const lotVigilances = vigilances.filter((v) => v.id.includes(lot.id));
        const hasLinks =
      ...
      <p className="flex items-start gap-1.5 text-xs text-muted-foreground [&_svg]:mt-0.5 [&_svg]:size-3.5 [&_svg]:shrink-0">
        <Lock aria-hidden />
        Le devis signé est immuable. En cas d'erreur, PHÉNIX la signale (vigilance) ou propose un
        avenant — il ne modifie jamais le document signé.
      </p>
    </div>
  );
}
